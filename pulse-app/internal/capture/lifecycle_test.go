...
func TestNormalizeLifecycleEventRejectsAuthorityFields(t *testing.T) {
	...
	for _, field := range []string{"team_id", "vault", "scope", "role", "audience", "visibility"} {
		input := map[string]any{}
...
func TestLifecycleStateTransitionsAndProvenanceClosure(t *testing.T) {
	...
	if err := ValidateCommonsProvenance([]ProvenanceRef{{VaultClass: VaultCommons, ObjectID: "commons-1"}}); err != nil {
		t.Fatalf("Commons provenance should pass: %v", err)
	}
	if err := ValidateCommonsProvenance([]ProvenanceRef{{VaultClass: VaultDesk, ObjectID: "desk-1"}}); err == nil || !strings.Contains(err.Error(), "private_lineage_forbidden") {
		t.Fatalf("expected private lineage rejection, got %v", err)
	}
}
...
func TestBindingLeaseAirlockAndMandatoryContracts(t *testing.T) {
	...
	team := BindingDecision{
		Schema: BindingSchema, Workspace: "/workspace/team", Kind: BindingTeam,
		TeamDeployment: "deployment-1", ReadVaults: []VaultClass{VaultDesk, VaultCommons},
		WriteDestination: DestinationDesk,
	}
	if err := ValidateBindingDecision(team); err != nil {
		t.Fatalf("team binding: %v", err)
	}
	team.WriteDestination = DestinationPersonal
	if err := ValidateBindingDecision(team); err == nil || !strings.Contains(err.Error(), "binding_destination_mismatch") {
	...
	if err := ValidateAirlockApproval("sha256:prepared", "sha256:changed"); err == nil || !strings.Contains(err.Error(), "airlock_digest_mismatch") {
