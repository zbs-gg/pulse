-- Content-free, append-only proof of the exact continuity payload offered to
-- a trusted host, its later host observation, and optional verified provider
-- usage. Object/evidence references are normalized so these tables have no
-- generic JSON or content-bearing column. Personal and Desk stores only.

CREATE TABLE continuity_delivery_receipts (
    ...
    parent_receipt_id           TEXT REFERENCES continuity_delivery_receipts(receipt_id) ON DELETE RESTRICT,
...
-- Memory Home first identifies recent contexts inside one immutable vault
-- boundary, then reads their append-only receipt chains in sequence order.
...
-- Bound the Home context window on real task starts before loading complete
-- receipt chains, so high-volume subagent contexts cannot displace proof.
...
-- Home attempt feeds start from the exact runtime binding and perform bounded
-- latest-receipt lookups. Rejected writes are kept on their own partial path.
CREATE INDEX idx_turn_ledgers_memory_home_binding
    ON turn_ledgers(binding_digest, ledger_id);
CREATE INDEX idx_memory_write_receipts_candidate_latest
    ON memory_write_receipts(candidate_id);
CREATE INDEX idx_memory_write_receipts_memory_home_rejected
    ON memory_write_receipts(ledger_id, created_at DESC, receipt_id DESC)
 WHERE status='rejected' AND candidate_id IS NULL;

CREATE TABLE continuity_delivery_object_refs (
    receipt_id TEXT NOT NULL REFERENCES continuity_delivery_receipts(receipt_id) ON DELETE RESTRICT,
...
CREATE TABLE continuity_delivery_evidence_refs (
    receipt_id TEXT NOT NULL REFERENCES continuity_delivery_receipts(receipt_id) ON DELETE RESTRICT,
    ordinal    INTEGER NOT NULL CHECK(ordinal BETWEEN 0 AND 63),
    ref_id     TEXT NOT NULL CHECK(
                   length(ref_id) BETWEEN 1 AND 256
...
-- A receipt and its normalized refs are written in one transaction. The seal
-- is appended only after every committed ref is present. Child-table INSERT
-- triggers below reject any later attempt to fill an otherwise unused ordinal,
-- while the reader verifies both counts and the canonical manifest digest.
CREATE TABLE continuity_delivery_ref_seals (
    receipt_id           TEXT PRIMARY KEY REFERENCES continuity_delivery_receipts(receipt_id) ON DELETE RESTRICT,
    refs_manifest_digest TEXT NOT NULL CHECK(
                             length(refs_manifest_digest) = 64
                             AND refs_manifest_digest NOT GLOB '*[^a-f0-9]*'
                         )
...
UPDATE store_identity
   SET min_reader_version = 46,
       min_writer_version = 46
 WHERE singleton = 1;
