-- Admit OpenCode as a truthful continuity delivery host while preserving the
-- immutable existing receipt rows and keeping provider measurement sources closed
...
CREATE TABLE continuity_delivery_receipts_v63 (
    ...
    parent_receipt_id            TEXT REFERENCES continuity_delivery_receipts_v63(receipt_id) ON DELETE RESTRICT,
    ...
    host                         TEXT NOT NULL CHECK(host IN ('codex', 'claude-code', 'cursor', 'opencode')),
...
INSERT INTO continuity_delivery_receipts_v63 (
...
CREATE TABLE continuity_delivery_object_refs_v63 (
    receipt_id TEXT NOT NULL REFERENCES continuity_delivery_receipts_v63(receipt_id) ON DELETE RESTRICT,
...
INSERT INTO continuity_delivery_object_refs_v63(receipt_id, ordinal, ref_id)
SELECT receipt_id, ordinal, ref_id FROM continuity_delivery_object_refs;

CREATE TABLE continuity_delivery_evidence_refs_v63 (
    receipt_id TEXT NOT NULL REFERENCES continuity_delivery_receipts_v63(receipt_id) ON DELETE RESTRICT,
    ordinal    INTEGER NOT NULL CHECK(ordinal BETWEEN 0 AND 63),
    ref_id     TEXT NOT NULL CHECK(
                   length(ref_id) BETWEEN 1 AND 256
...
INSERT INTO continuity_delivery_evidence_refs_v63(receipt_id, ordinal, ref_id)
SELECT receipt_id, ordinal, ref_id FROM continuity_delivery_evidence_refs;

CREATE TABLE continuity_delivery_ref_seals_v63 (
    receipt_id           TEXT PRIMARY KEY REFERENCES continuity_delivery_receipts_v63(receipt_id) ON DELETE RESTRICT,
    refs_manifest_digest TEXT NOT NULL CHECK(
                             length(refs_manifest_digest) = 64
                             AND refs_manifest_digest NOT GLOB '*[^a-f0-9]*'
                         )
) WITHOUT ROWID;
INSERT INTO continuity_delivery_ref_seals_v63(receipt_id, refs_manifest_digest)
...
ALTER TABLE continuity_delivery_receipts_v63 RENAME TO continuity_delivery_receipts;
ALTER TABLE continuity_delivery_object_refs_v63 RENAME TO continuity_delivery_object_refs;
ALTER TABLE continuity_delivery_evidence_refs_v63 RENAME TO continuity_delivery_evidence_refs;
ALTER TABLE continuity_delivery_ref_seals_v63 RENAME TO continuity_delivery_ref_seals;
...
UPDATE store_identity
   SET min_reader_version = 45,
       min_writer_version = 45
 WHERE singleton = 1
