...
func openContinuityDeliveryStore(t *testing.T) *Store {
	t.Helper()
	s, err := OpenVault(filepath.Join(t.TempDir(), "desk.db"), StoreKindDesk, "store_desk_delivery")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = s.Close() })
	if err := s.ConfigureContinuityDeliveryAuthority(
		localStoreBindingDigest("store_desk_delivery"), "repository_pulse",
	); err != nil {
		t.Fatal(err)
	}
	return s
...
func testContinuityOfferRequest() ContinuityDeliveryOfferRequest {
	req := ContinuityDeliveryOfferRequest{
		...
		BindingDigest: localStoreBindingDigest("store_desk_delivery"), RepositoryID: "repository_pulse",
...
func TestMigration046AppliesOnlyToPersonalAndDeskWithContentFreeClosedSchema(t *testing.T) {
	desk, err := OpenVault(filepath.Join(t.TempDir(), "desk.db"), StoreKindDesk, "store_desk_delivery")
	if err != nil {
		t.Fatal(err)
	}
	defer desk.Close()
	...
	if err := desk.DB().QueryRow(`
		SELECT disposition FROM schema_migration_applicability WHERE version=46`,
	).Scan(&disposition); err != nil || disposition != "applied" {
		t.Fatalf("desk v46 disposition=%q err=%v", disposition, err)
	}
	for _, table := range []string{
		...
		if err := desk.DB().QueryRow(`
			...
			t.Fatalf("desk table %s exists=%d err=%v", table, exists, err)
		}
	}
	for index, want := range map[string]string{
		...
		indexRows, err := desk.DB().Query(`PRAGMA index_info(` + index + `)`)
		if err != nil {
			t.Fatal(err)
		}
	...
	rows, err := desk.DB().Query(`PRAGMA table_info(continuity_delivery_receipts)`)
	if err != nil {
		t.Fatal(err)
	}
	defer rows.Close()
	...
	if err := preview.DB().QueryRow(`
		...
		t.Fatalf("preview v46 disposition=%q err=%v", disposition, err)
	}
	var exists int
	if err := preview.DB().QueryRow(`
	...
	policy, ok := postFoundationMigrationPolicies[46]
	if !ok || policy.StoreKinds[StoreKindCommons] || policy.StoreKinds[StoreKindLocalPreview] {
		t.Fatalf("v46 policy is not Personal/Desk-only: %#v", policy.StoreKinds)
	}
}
