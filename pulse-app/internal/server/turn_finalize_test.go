...
func newProductMemoryServer(t *testing.T) (*store.Store, *httptest.Server) {
	...
	vault, err := store.OpenVault(
		filepath.Join(t.TempDir(), "personal.db"), store.StoreKindPersonal, "store_personal_server_test",
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestProductCommitRecallDeleteInvalidatesLiveRetrievalImmediately(t *testing.T) {
	vault, err := store.OpenVault(
		filepath.Join(t.TempDir(), "personal-live-retrieval.db"), store.StoreKindPersonal, "store_personal_live_retrieval_test",
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSemanticDeleteRebuildsAndReindexesRemainingPrivateContributions(t *testing.T) {
	vault, err := store.OpenVault(
		filepath.Join(t.TempDir(), "personal-semantic-delete.db"), store.StoreKindPersonal, "store_personal_semantic_delete_test",
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestProjectionStaysPendingWithoutEmbedderAndReplaysWhenEnabled(t *testing.T) {
	path := filepath.Join(t.TempDir(), "personal-deferred-projection.db")
	vault, err := store.OpenVault(path, store.StoreKindPersonal, "store_personal_deferred_projection_test")
	if err != nil {
		t.Fatal(err)
	}
	now := time.Now().UTC()
	...
	reopened, err := store.OpenVault(path, store.StoreKindPersonal, "store_personal_deferred_projection_test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSemanticRebuildWithoutEmbedderReplaysSurvivorAfterRestart(t *testing.T) {
	path := filepath.Join(t.TempDir(), "personal-semantic-restart.db")
	vault, err := store.OpenVault(path, store.StoreKindPersonal, "store_personal_semantic_restart_test")
	if err != nil {
		t.Fatal(err)
	}
	engine := retrieve.New(retrieve.Config{Store: vault, Embedder: productTestEmbedder{}})
	...
	reopened, err := store.OpenVault(path, store.StoreKindPersonal, "store_personal_semantic_restart_test")
	if err != nil {
		t.Fatal(err)
	}
	defer reopened.Close()
...
func TestProjectionRetriesTransientEmbedderFailureWithoutRestart(t *testing.T) {
	vault, err := store.OpenVault(
		filepath.Join(t.TempDir(), "personal-projection-retry.db"), store.StoreKindPersonal, "store_personal_projection_retry_test",
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestProductServerCommitsImmediatelyWithoutPresentation(t *testing.T) {
	vault, err := store.OpenVault(
		filepath.Join(t.TempDir(), "personal-auto.db"), store.StoreKindPersonal, "store_personal_auto_test",
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestProductServerRecoveryTurnsInvalidCandidateIntoDurableFailure(t *testing.T) {
	vault, err := store.OpenVault(
		filepath.Join(t.TempDir(), "personal-auto-fail.db"), store.StoreKindPersonal, "store_personal_auto_fail_test",
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestProductServerRestartCommitsPendingCandidateWithoutPresentation(t *testing.T) {
	path := filepath.Join(t.TempDir(), "personal-restart.db")
	vault, err := store.OpenVault(path, store.StoreKindPersonal, "store_personal_restart_test")
	if err != nil {
		t.Fatal(err)
	}
	if err := vault.ConfigureProductRuntimeAuthority(
	...
	reopened, err := store.OpenVault(path, store.StoreKindPersonal, "store_personal_restart_test")
	if err != nil {
		t.Fatal(err)
	}
	defer reopened.Close()
...
func TestRestartFailsCandidateFromStaleRuntimeAuthority(t *testing.T) {
	path := filepath.Join(t.TempDir(), "personal-restart-stale-authority.db")
	vault, err := store.OpenVault(path, store.StoreKindPersonal, "store_personal_restart_stale_test")
	if err != nil {
		t.Fatal(err)
	}
	...
	reopened, err := store.OpenVault(path, store.StoreKindPersonal, "store_personal_restart_stale_test")
	if err != nil {
		t.Fatal(err)
	}
	defer reopened.Close()
...
func TestProductServerRestartRecoversMoreThanOnePendingPage(t *testing.T) {
	path := filepath.Join(t.TempDir(), "personal-restart-many.db")
	vault, err := store.OpenVault(path, store.StoreKindPersonal, "store_personal_restart_many_test")
	if err != nil {
		t.Fatal(err)
	}
	if err := vault.ConfigureProductRuntimeAuthority(
	...
	reopened, err := store.OpenVault(path, store.StoreKindPersonal, "store_personal_restart_many_test")
	if err != nil {
		t.Fatal(err)
	}
	defer reopened.Close()
