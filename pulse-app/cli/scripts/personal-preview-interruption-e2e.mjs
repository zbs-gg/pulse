...
function artifact(bytes) {
  return {
    id: 'pulse-daemon', kind: 'daemon', version: '0.7.0', epoch: 7,
    url: 'https://releases.zbs.gg/pulse/0.7.0/daemon.dmg',
...
try {
  ...
  assert.equal(existsSync(join(packedRoot, 'src', 'git-team-memory.js')), true);
  assert.equal(existsSync(join(packedRoot, 'src', 'project-source.js')), true);
