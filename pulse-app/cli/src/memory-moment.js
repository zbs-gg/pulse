...
export function validateMomentItems(items) {
    if (!Array.isArray(items) || !items.length)
        throw new Error('memory_items_required_nothing_accepted');
    return items.map(item => {
        if (!item || typeof item !== 'object' || Array.isArray(item) ||
            ...
            (item.privacy !== undefined && !['normal', 'sensitive', 'private'].includes(item.privacy)))
            throw new Error('memory_item_invalid_nothing_accepted');
        ...
        if (['/users/', 'file://', 'token=', 'api_key', 'apikey', 'password', 'secret', 'private_key', 'begin private key', 'sk-', 'akia', 'xoxb-', 'ghp_'].some(marker => lower.includes(marker)) ||
            ...
            throw new Error('unsafe_memory_nothing_accepted');
        const emotions = item.emotions ?? (item.emotion ? [item.emotion] : []);
        if (item.kind === 'emotion') {
            if (!Array.isArray(emotions) || !emotions.length || (item.emotions && item.emotion))
                throw new Error('memory_emotions_required_nothing_accepted');
            for (const e of emotions)
                if (!e || Object.keys(e).some(k => !['label', 'name', 'intensity', 'source', 'cause'].includes(k)) || !labels.has(e.label) || !Number.isFinite(e.intensity) || e.intensity < 0 || e.intensity > 1 || !['user', 'inferred'].includes(e.source) ||
                    (e.name !== undefined && (typeof e.name !== 'string' || !e.name.trim() || Buffer.byteLength(e.name) > 120)) ||
                    (e.cause !== undefined && (typeof e.cause !== 'string' || !e.cause.trim() || Buffer.byteLength(e.cause) > 360)))
                    throw new Error('memory_emotion_invalid_nothing_accepted');
        }
        else if (item.emotion !== undefined || item.emotions !== undefined)
            throw new Error('memory_emotions_unexpected_nothing_accepted');
        return item;
    });
}
export function expandMomentItems(items) {
    return items.flatMap(item => {
        ...
        for (const c of item.summary) {
            if (Buffer.byteLength(part + c) > 1100) {
                parts.push(part);
                part = '';
            }
            part += c;
...
export function compactMomentResult(result, expected) {
    ...
    if (!result?.ledger_id || !Array.isArray(receipts) || !receipts.length)
...
export async function writeMemoryMoment(request, body, signal) {
    try {
        return await request('/memory/moments', { body, timeoutMs: 5000, idempotencyKey: body.idempotency_key });
    }
    catch (error) {
        if (signal?.aborted || (error.status >= 400 && error.status < 500) || /(?:HTTP |http_)4\d\d/.test(error.message ?? ''))
            throw error;
