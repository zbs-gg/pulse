...
for (const size of [4, 21, 61])
    test(`all ${size} meaningful parts survive construction`, () => {
...
test('lost response replays identical request once; validation never retries', async () => {
    ...
    const calls = [];
    const answer = await writeMemoryMoment(async (path, options) => { calls.push(options.body); if (calls.length === 1)
        throw new Error('connection reset'); return { ok: true }; }, body);
