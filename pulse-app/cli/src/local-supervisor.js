...
export function vaultRuntimeFromBinding(binding) {
  ...
  const vault = binding.mode === 'personal' ? binding.personal : undefined;
  const kind = binding.mode === 'personal' ? 'personal' : undefined;
  if (!vault || !kind) {
    throw new SupervisorError('vault_runtime_invalid', 'binding has no Personal vault');
  }
