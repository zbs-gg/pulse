...
const productEnvironment = resolveProductEnvironment({ host: 'cursor', integrity: 'reuse' });
...
const child = spawn(process.execPath, [cliPath, 'cursor-mcp'], {
  stdio: 'inherit',
...
child.on('error', (error) => {
  process.stderr.write(`[pulse] Cursor MCP launcher failed: ${error.message}\n`);
  process.exitCode = 1;
});
