...
export interface ConsolidationReport {
  ...
  generation: number;
  destination: { store_kind: 'personal' | 'desk'; store_id: string; binding_digest: string; repository_id: string };
...
export function validateConsolidationReport(rawReport: unknown): ConsolidationReport {
  ...
  if (!exactKeys(destination, ['store_kind', 'store_id', 'binding_digest', 'repository_id']) ||
      (destination.store_kind !== 'personal' && destination.store_kind !== 'desk') ||
