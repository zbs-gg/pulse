...
func (c *Client) Chat(ctx context.Context, req model.ChatRequest, resolved model.ResolvedModel) (*model.ChatResponse, error) {
	if resolved.APIKey == "" {
		return nil, fmt.Errorf("missing DO token: %w", model.ErrDORequiredUnavailable)
	}
	...
	url := strings.TrimRight(resolved.BaseURL, "/") + "/chat/completions"
	...
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("do inference %d: %s: %w", resp.StatusCode, string(respBody), model.ErrDORequiredUnavailable)
	}
	var parsed chatResp
	if err := json.Unmarshal(respBody, &parsed); err != nil {
		return nil, fmt.Errorf("unmarshal DO response: %w", err)
	}
	text := ""
	stop := ""
...
func (c *Client) Health(ctx context.Context, resolved model.ResolvedModel) error {
	if resolved.APIKey == "" {
		return fmt.Errorf("missing DO token: %w", model.ErrDORequiredUnavailable)
	}
	...
	defer resp.Body.Close()
	if resp.StatusCode >= 400 {
		return fmt.Errorf("do inference health %d: %w", resp.StatusCode, model.ErrDORequiredUnavailable)
	}
	return nil
}
